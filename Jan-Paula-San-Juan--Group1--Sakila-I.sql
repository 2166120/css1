...
SELECT email, char_length(email) AS email_length FROM customer ORDER BY email_length DESC, email ASC;
/**13. Now retrieve top 100 customer records with the shortest email address.**/
SELECT email, char_length(email) AS email_length FROM customer ORDER BY email_length ASC LIMIT 100;